...
public class VagaController {
		...
		@PostMapping
		public ResponseEntity<Object> registroDaVaga(
						@RequestBody
						@Valid VagaDto vagaDto) {
		...
		@GetMapping("/{id}")
		public ResponseEntity<Object> getOneVagas(
		...
		public ResponseEntity<Object> deleteVagas(
				...
				Optional<VagasModel> vagasModelOptional = vagasServices.findById(id);
				if (!vagasModelOptional.isPresent()) {
						return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Vaga liberada com sucesso!!");
				}
				vagasServices.delete(vagasModelOptional.get());
				return ResponseEntity.status(HttpStatus.OK).body(vagasModelOptional.get());
		}

		@PutMapping("/{id}")
		public ResponseEntity<Object>atualizacaoDeVaga(@PathVariable(value = "id")UUID id,
				...
				return ResponseEntity.status(HttpStatus.OK).body(vagasServices.save(vagasModel));
		}
